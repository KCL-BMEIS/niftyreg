...
for d=2:3
    ...
    for type={'nearest', 'linear', 'cubic'}
        ...
        expectedWarpedImage = zeros(inputDef.hdr.dime.dim(2),inputDef.hdr.dime.dim(3),inputDef.hdr.dime.dim(4),'single');
        
        if d==2
            [X,Y] = ndgrid((0:size(inputData,1)-1),(0:size(inputData,2)-1));
            X=single(X);
            Y=single(Y);
        elseif d==3
            [X,Y,Z] = ndgrid((0:size(inputData,1)-1),(0:size(inputData,2)-1),(0:size(inputData,3)-1));
            X=single(X);
            Y=single(Y);
            Z=single(Z);
        end
        for kk=1:size(expectedWarpedImage,3)
            for jj=1:size(expectedWarpedImage,2)
                for ii=1:size(expectedWarpedImage,1)
                    if d==2
                        currentWorldPosition = [inputDefData(ii,jj,kk,1,1) inputDefData(ii,jj,kk,1,2) 0 1];
                    elseif d==3
                        currentWorldPosition = [inputDefData(ii,jj,kk,1,1) inputDefData(ii,jj,kk,1,2) inputDefData(ii,jj,kk,1,3) 1];
                    end
                    correspondingVoxelPosition = single(double(worldToVoxelMatrix) * double(currentWorldPosition)');
                    if d==2
                        intensityValue = interpn(double(X),double(Y),double(inputData),...
                            double(correspondingVoxelPosition(1)),...
                            double(correspondingVoxelPosition(2)),...
                            type{1}, -999999);
                    elseif d==3
                        intensityValue = interpn(double(X),double(Y),double(Z),double(inputData),...
                            double(correspondingVoxelPosition(1)),...
                            double(correspondingVoxelPosition(2)),...

...
int main(int argc, char **argv)
{
    ...
    if (argc != 5) {
        fprintf(stderr, "Usage: %s <inputPoints1> <inputPoints2> <isAffine> <expectedLTSMatrix> \n", argv[0]);
        return EXIT_FAILURE;
    }

    char *inputMatrix1Filename = argv[1];
    char *inputMatrix2Filename = argv[2];
    bool isAffine = atoi(argv[3]);
    char *expectedLTSMatrixFilename = argv[4];
...
#ifndef NDEBUG
            reg_mat44_disp(test_leastSquares, (char *) "test_affine_transformation2D");
#endif
            if (check_matrix_difference(*expectedLSMatrix, *test_leastSquares, (char *) "LS matrices 2D affine", max_difference)) return EXIT_FAILURE;
        }
        else {
            estimate_rigid_transformation2D(points, test_leastSquares);
#ifndef NDEBUG
            reg_mat44_disp(test_leastSquares, (char *) "test_rigid_transformation2D");
#endif
            if (check_matrix_difference(*expectedLSMatrix, *test_leastSquares, (char *) "LS matrices 2D rigid", max_difference)) return EXIT_FAILURE;
        }
...
#ifndef NDEBUG
            reg_mat44_disp(test_leastSquares, (char *) "test_affine_transformation3D");
#endif
            if (check_matrix_difference(*expectedLSMatrix, *test_leastSquares, (char *) "LS matrices 3D affine", max_difference)) return EXIT_FAILURE;
        }
        else {
            estimate_rigid_transformation3D(points, test_leastSquares);
#ifndef NDEBUG
            reg_mat44_disp(test_leastSquares, (char *) "test_rigid_transformation3D");
#endif
            if (check_matrix_difference(*expectedLSMatrix, *test_leastSquares, (char *) "LS matrices 3D rigid", max_difference)) return EXIT_FAILURE;
        }

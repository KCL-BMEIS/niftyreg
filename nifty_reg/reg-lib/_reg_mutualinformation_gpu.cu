                             ...
                             int *mask)
{
    ...
    dim3 G2((int)ceil((float)(target_bins[0]*result_bins[0]*result_bins[1])/(float)B1.x),1,1);
    reg_smoothJointHistogramY_kernel <<< G2, B1 >>> (*logJointHistogram_d);
    CUDA_SAFE_CALL(cudaThreadSynchronize());
#ifndef NDEBUG
    printf("[NiftyReg CUDA DEBUG] reg_smoothJointHistogramY_kernel: %s - Grid size [%i %i %i] - Block size [%i %i %i]\n",
           cudaGetErrorString(cudaGetLastError()),G2.x,G2.y,G2.z,B1.x,B1.y,B1.z);
#endif

    // The joint histogram is smoothed along the z axis
    CUDA_SAFE_CALL(cudaBindTexture(0, histogramTexture, *logJointHistogram_d, binNumber*sizeof(float)));
    dim3 G3((int)ceil((float)(target_bins[0]*target_bins[1]*result_bins[1])/(float)B1.x),1,1);
    reg_smoothJointHistogramZ_kernel <<< G3, B1 >>> (tempHistogram);
    CUDA_SAFE_CALL(cudaThreadSynchronize());
#ifndef NDEBUG
    printf("[NiftyReg CUDA DEBUG] reg_smoothJointHistogramZ_kernel: %s - Grid size [%i %i %i] - Block size [%i %i %i]\n",
           cudaGetErrorString(cudaGetLastError()),G3.x,G3.y,G3.z,B1.x,B1.y,B1.z);
#endif

    // The joint histogram is smoothed along the w axis
    CUDA_SAFE_CALL(cudaBindTexture(0, histogramTexture, tempHistogram, binNumber*sizeof(float)));
    dim3 G4((int)ceil((float)(target_bins[0]*target_bins[1]*result_bins[0])/(float)B1.x),1,1);
    reg_smoothJointHistogramW_kernel <<< G4, B1 >>> (*logJointHistogram_d);
    CUDA_SAFE_CALL(cudaThreadSynchronize());
#ifndef NDEBUG
    printf("[NiftyReg CUDA DEBUG] reg_smoothJointHistogramW_kernel: %s - Grid size [%i %i %i] - Block size [%i %i %i]\n",
           cudaGetErrorString(cudaGetLastError()),G4.x,G4.y,G4.z,B1.x,B1.y,B1.z);
#endif
    ...
    CUDA_SAFE_CALL(cudaBindTexture(0, histogramTexture, *logJointHistogram_d, binNumber*sizeof(float)));
    dim3 G5((int)ceil((float)(target_bins[1]*result_bins[0]*result_bins[1])/(float)B1.x),1,1);
    reg_marginaliseTargetX_kernel <<< G5, B1 >>> (temp3DHistogram);
    CUDA_SAFE_CALL(cudaThreadSynchronize());
#ifndef NDEBUG
    printf("[NiftyReg CUDA DEBUG] reg_marginaliseTargetX_kernel: %s - Grid size [%i %i %i] - Block size [%i %i %i]\n",
           cudaGetErrorString(cudaGetLastError()),G5.x,G5.y,G5.z,B1.x,B1.y,B1.z);
#endif
    ...
    dim3 G6((int)ceil((float)(result_bins[0]*result_bins[1])/(float)B1.x),1,1);
    reg_marginaliseTargetXY_kernel <<< G6, B1 >>> (temp2DHistogram);
    CUDA_SAFE_CALL(cudaThreadSynchronize());
#ifndef NDEBUG
    printf("[NiftyReg CUDA DEBUG] reg_marginaliseTargetXY_kernel: %s - Grid size [%i %i %i] - Block size [%i %i %i]\n",
           cudaGetErrorString(cudaGetLastError()),G6.x,G6.y,G6.z,B1.x,B1.y,B1.z);
#endif    
    ...
    CUDA_SAFE_CALL(cudaBindTexture(0, histogramTexture, *logJointHistogram_d, binNumber*sizeof(float)));
    dim3 G7((int)ceil((float)(target_bins[0]*target_bins[1]*result_bins[0])/(float)B1.x),1,1);
    reg_marginaliseResultX_kernel <<< G7, B1 >>> (temp3DHistogram);
    CUDA_SAFE_CALL(cudaThreadSynchronize());
#ifndef NDEBUG
    printf("[NiftyReg CUDA DEBUG] reg_marginaliseResultX_kernel: %s - Grid size [%i %i %i] - Block size [%i %i %i]\n",
           cudaGetErrorString(cudaGetLastError()),G7.x,G7.y,G7.z,B1.x,B1.y,B1.z);
#endif
    ...
    dim3 G8((int)ceil((float)(target_bins[0]*target_bins[1])/(float)B1.x),1,1);
    reg_marginaliseResultXY_kernel <<< G8, B1 >>> (temp2DHistogram);
    CUDA_SAFE_CALL(cudaThreadSynchronize());
#ifndef NDEBUG
    printf("[NiftyReg CUDA DEBUG] reg_marginaliseResultXY_kernel: %s - Grid size [%i %i %i] - Block size [%i %i %i]\n",
           cudaGetErrorString(cudaGetLastError()),G8.x,G8.y,G8.z,B1.x,B1.y,B1.z);
#endif

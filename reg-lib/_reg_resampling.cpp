...
void interpWindowedSincKernel(double relative, double *basis) {
	...
	for (int i = -SINC_KERNEL_RADIUS; i < SINC_KERNEL_RADIUS; ++i) {
		...
		if (x == 0.0)
			...
			double pi_x = M_PI * x;
			basis[j] = static_cast<double>(SINC_KERNEL_RADIUS) * sin(pi_x)
...
// by the the log tensor components
	if (dtIndicies[0] != -1) {
		...
		size_t floatingVoxelNumber = (size_t) floatingImage->nx
				* floatingImage->ny * floatingImage->nz;
#endif
		*originalFloatingData = (void *) malloc(
				floatingImage->nvox * sizeof(DTYPE));
		memcpy(*originalFloatingData, floatingImage->data,
				floatingImage->nvox * sizeof(DTYPE));
#ifndef NDEBUG
...
// CAUTION: Here the tensor is assumed to be encoding in lower triangular order
		DTYPE *floatingIntensityXX = &firstVox[floatingVoxelNumber
				* dtIndicies[0]];
		DTYPE *floatingIntensityXY = &firstVox[floatingVoxelNumber
				* dtIndicies[1]];
		DTYPE *floatingIntensityYY = &firstVox[floatingVoxelNumber
				* dtIndicies[2]];
		DTYPE *floatingIntensityXZ = &firstVox[floatingVoxelNumber
				* dtIndicies[3]];
		DTYPE *floatingIntensityYZ = &firstVox[floatingVoxelNumber
				* dtIndicies[4]];
		DTYPE *floatingIntensityZZ = &firstVox[floatingVoxelNumber
				* dtIndicies[5]];
...
floatingIntensityYZ)
#endif
		for (floatingIndex = 0; floatingIndex < floatingVoxelNumber;
				++floatingIndex) {
...
// Write this out as a new image
			floatingIntensityXX[floatingIndex] =
					static_cast<DTYPE>(diffTensor.m[0][0]);
			floatingIntensityXY[floatingIndex] =
					static_cast<DTYPE>(diffTensor.m[0][1]);
			floatingIntensityYY[floatingIndex] =
					static_cast<DTYPE>(diffTensor.m[1][1]);
			floatingIntensityXZ[floatingIndex] =
					static_cast<DTYPE>(diffTensor.m[0][2]);
			floatingIntensityYZ[floatingIndex] =
					static_cast<DTYPE>(diffTensor.m[1][2]);
			floatingIntensityZZ[floatingIndex] =
					static_cast<DTYPE>(diffTensor.m[2][2]);
		}
#ifndef NDEBUG
...
#else
		size_t warpedIndex;
		size_t voxelNumber = (size_t) inputImage->nx * inputImage->ny
				* inputImage->nz;
#endif
		DTYPE *warpVox, *warpedXX, *warpedXY, *warpedXZ, *warpedYY, *warpedYZ,
				*warpedZZ;
		if (warpedImage != NULL) {
...
// CAUTION: Here the tensor is assumed to be encoding in lower triangular order
			...
			DTYPE *inputIntensityXX = &firstWarpVox[voxelNumber
					* (dtIndicies[0] + inputImage->nt * u)];
			DTYPE *inputIntensityXY = &firstWarpVox[voxelNumber
					* (dtIndicies[1] + inputImage->nt * u)];
			DTYPE *inputIntensityYY = &firstWarpVox[voxelNumber
					* (dtIndicies[2] + inputImage->nt * u)];
			DTYPE *inputIntensityXZ = &firstWarpVox[voxelNumber
					* (dtIndicies[3] + inputImage->nt * u)];
			DTYPE *inputIntensityYZ = &firstWarpVox[voxelNumber
					* (dtIndicies[4] + inputImage->nt * u)];
			DTYPE *inputIntensityZZ = &firstWarpVox[voxelNumber
					* (dtIndicies[5] + inputImage->nt * u)];
...
// Fill the rest of the mat44 with the tensor components
					inputTensor.m[0][0] =
							static_cast<double>(inputIntensityXX[warpedIndex]);
					inputTensor.m[0][1] =
							static_cast<double>(inputIntensityXY[warpedIndex]);
					inputTensor.m[1][0] = inputTensor.m[0][1];
					inputTensor.m[1][1] =
							static_cast<double>(inputIntensityYY[warpedIndex]);
					inputTensor.m[0][2] =
							static_cast<double>(inputIntensityXZ[warpedIndex]);
					inputTensor.m[2][0] = inputTensor.m[0][2];
					inputTensor.m[1][2] =
							static_cast<double>(inputIntensityYZ[warpedIndex]);
					inputTensor.m[2][1] = inputTensor.m[1][2];
					inputTensor.m[2][2] =
							static_cast<double>(inputIntensityZZ[warpedIndex]);
// Exponentiate the warped tensor
					if (warpedImage == NULL) {
						...
						warpedTensor.m[0][0] =
								static_cast<double>(warpedXX[warpedIndex]);
						warpedTensor.m[0][1] =
								static_cast<double>(warpedXY[warpedIndex]);
						warpedTensor.m[1][0] = warpedTensor.m[0][1];
						warpedTensor.m[1][1] =
								static_cast<double>(warpedYY[warpedIndex]);
						warpedTensor.m[0][2] =
								static_cast<double>(warpedXZ[warpedIndex]);
						warpedTensor.m[2][0] = warpedTensor.m[0][2];
						warpedTensor.m[1][2] =
								static_cast<double>(warpedYZ[warpedIndex]);
						warpedTensor.m[2][1] = warpedTensor.m[1][2];
						warpedTensor.m[2][2] =
								static_cast<double>(warpedZZ[warpedIndex]);
						inputTensor = nifti_mat33_mul(warpedTensor,
								inputTensor);
						testSum = static_cast<double>(warpedTensor.m[0][0]
								+ warpedTensor.m[0][1] + warpedTensor.m[0][2]
								+ warpedTensor.m[1][0] + warpedTensor.m[1][1]
								+ warpedTensor.m[1][2] + warpedTensor.m[2][0]
								+ warpedTensor.m[2][1] + warpedTensor.m[2][2]);
					}
					if (testSum == testSum) {
...
// We need both the rotation matrix, and it's transpose
						for (col = 0; col < 3; col++) {
							for (row = 0; row < 3; row++) {
								RotMat.m[col][row] =
										static_cast<double>(R.m[col][row]);
								RotMatT.m[col][row] =
										static_cast<double>(R.m[row][col]);
							}
						}
// As the mat44 multiplication uses pointers, do the multiplications separately
						inputTensor = nifti_mat33_mul(
								nifti_mat33_mul(RotMatT, inputTensor), RotMat);
// Finally, read the tensor back out as a warped image
						inputIntensityXX[warpedIndex] =
								static_cast<DTYPE>(inputTensor.m[0][0]);
						inputIntensityYY[warpedIndex] =
								static_cast<DTYPE>(inputTensor.m[1][1]);
						inputIntensityZZ[warpedIndex] =
								static_cast<DTYPE>(inputTensor.m[2][2]);
						inputIntensityXY[warpedIndex] =
								static_cast<DTYPE>(inputTensor.m[0][1]);
						inputIntensityXZ[warpedIndex] =
								static_cast<DTYPE>(inputTensor.m[0][2]);
						inputIntensityYZ[warpedIndex] =
								static_cast<DTYPE>(inputTensor.m[1][2]);
					} else {
						inputIntensityXX[warpedIndex] = std::numeric_limits<
								DTYPE>::quiet_NaN();
						inputIntensityYY[warpedIndex] = std::numeric_limits<
								DTYPE>::quiet_NaN();
						inputIntensityZZ[warpedIndex] = std::numeric_limits<
								DTYPE>::quiet_NaN();
						inputIntensityXY[warpedIndex] = std::numeric_limits<
								DTYPE>::quiet_NaN();
						inputIntensityXZ[warpedIndex] = std::numeric_limits<
								DTYPE>::quiet_NaN();
						inputIntensityYZ[warpedIndex] = std::numeric_limits<
								DTYPE>::quiet_NaN();
					}
				}
			}
...
void ResampleImage3D(nifti_image *floatingImage, nifti_image *deformationField,
		nifti_image *warpedImage, int *mask, FieldTYPE paddingValue,
		int kernel) {
#ifdef _WIN32
	long index;
	...
	size_t floatingVoxelNumber = (size_t) floatingImage->nx * floatingImage->ny
			* floatingImage->nz;
#endif
	FloatingTYPE *floatingIntensityPtr =
			static_cast<FloatingTYPE *>(floatingImage->data);
	FloatingTYPE *warpedIntensityPtr =
			static_cast<FloatingTYPE *>(warpedImage->data);
	FieldTYPE *deformationFieldPtrX =
			static_cast<FieldTYPE *>(deformationField->data);
...
// Compute the resolution of the progress bar
	...
	unsigned long nProgressSteps = warpedImage->nt * warpedImage->nu
			* warpedVoxelNumber;
	unsigned long progressUnit = (unsigned long) ceil(
			(float) nProgressSteps / 100.0f);
#endif
	int *maskPtr = &mask[0];
	...
	if (floatingImage->sform_code > 0)
		...
		floatingIJKMatrix = &(floatingImage->qto_ijk);
...
#endif
		FloatingTYPE *warpedIntensity = &warpedIntensityPtr[t
				* warpedVoxelNumber];
		FloatingTYPE *floatingIntensity = &floatingIntensityPtr[t
				* floatingVoxelNumber];
		double xBasis[SINC_KERNEL_SIZE], yBasis[SINC_KERNEL_SIZE],
				zBasis[SINC_KERNEL_SIZE], relative[3];
...
// real -> voxel; floating space
				...
				for (c = 0; c < kernel_size; c++) {
					Z = previous[2] + c;
					zPointer = &floatingIntensity[Z * floatingImage->nx
							* floatingImage->ny];
					yTempNewValue = 0.0;
					for (b = 0; b < kernel_size; b++) {
						...
						for (a = 0; a < kernel_size; a++) {
							if (-1 < (previous[0] + a)
									&& (previous[0] + a) < floatingImage->nx
									&& -1 < Z && Z < floatingImage->nz && -1 < Y
									&& Y < floatingImage->ny) {
								xTempNewValue +=
										static_cast<double>(*xyzPointer)
												* xBasis[a];
							} else {
// paddingValue
			...
			case NIFTI_TYPE_UINT8:
				...
				warpedIntensity[index] = static_cast<FloatingTYPE>(
						intensity > 0 ? reg_round(intensity) : 0);
				break;
			...
			case NIFTI_TYPE_UINT32:
				intensity = (
						intensity <= 4294967295 ?
								reg_round(intensity) : 4294967295); // 4294967295=2^32-1
				warpedIntensity[index] = static_cast<FloatingTYPE>(
						intensity > 0 ? reg_round(intensity) : 0);
				break;
			default:
...
/* *************************************************************** */
template<class FloatingTYPE, class FieldTYPE>
void ResampleImage2D(nifti_image *floatingImage, nifti_image *deformationField,
		nifti_image *warpedImage, int *mask, FieldTYPE paddingValue,
		int kernel) {
#ifdef _WIN32
	long index;
				...
				for (b = 0; b < kernel_size; b++) {
					...
					for (a = 0; a < kernel_size; a++) {
						if (-1 < (previous[0] + a)
							...
							xTempNewValue += (FieldTYPE) *xyzPointer
									* xBasis[a];
						} else {
// paddingValue
							xTempNewValue += paddingValue * xBasis[a];
				...
				switch (floatingImage->datatype) {
				case NIFTI_TYPE_FLOAT32:
					warpedIntensity[index] =
							static_cast<FloatingTYPE>(intensity);
					break;
				case NIFTI_TYPE_FLOAT64:
					warpedIntensity[index] = intensity;
					break;
				...
				case NIFTI_TYPE_UINT16:
					intensity = (
							intensity <= 65535 ? reg_round(intensity) : 65535); // 65535=2^16-1
					warpedIntensity[index] = static_cast<FloatingTYPE>(
							intensity > 0 ? reg_round(intensity) : 0);
					break;
				case NIFTI_TYPE_UINT32:
...
void reg_resampleImage2(nifti_image *floatingImage, nifti_image *warpedImage,
		nifti_image *deformationFieldImage, int *mask, int interp,
		FieldTYPE paddingValue, int* dtIndicies, mat33 * jacMat) {
...
// The DTI are logged
	reg_dti_resampling_preprocessing<FloatingTYPE>(floatingImage,
			&originalFloatingData, dtIndicies);
// The deformation field contains the position in the real world
	if (deformationFieldImage->nz > 1) {
		ResampleImage3D<FloatingTYPE, FieldTYPE>(floatingImage,
				deformationFieldImage, warpedImage, mask, paddingValue, interp);
	} else {
...
// The interpolated tensors are reoriented and exponentiated
	reg_dti_resampling_postprocessing<FloatingTYPE>(warpedImage, mask, jacMat,
			dtIndicies);
}
/* *************************************************************** */
void reg_resampleImage(nifti_image *floatingImage, nifti_image *warpedImage,
		nifti_image *deformationField, int *mask, int interp,
		float paddingValue, bool *dti_timepoint, mat33 * jacMat) {
	if (floatingImage->datatype != warpedImage->datatype) {
		printf(
				"[NiftyReg ERROR] reg_resampleImage\tfloating and warped image should have the same data type\n");
		printf("[NiftyReg ERROR] reg_resampleImage\tNothing has been done\n");
		reg_exit(1);
	}
	if (floatingImage->nt != warpedImage->nt) {
		printf(
				"[NiftyReg ERROR] reg_resampleImage\tThe floating and warped images have different dimension along the time axis\n");
		printf("[NiftyReg ERROR] reg_resampleImage\tNothing has been done\n");
		reg_exit(1);
	}
...
// voxels in the background are set to negative value so 0 corresponds to active voxel
	...
	case NIFTI_TYPE_FLOAT32:
		switch (floatingImage->datatype) {
		case NIFTI_TYPE_UINT8:
			reg_resampleImage2<float, unsigned char>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_INT8:
			reg_resampleImage2<float, char>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_UINT16:
			reg_resampleImage2<float, unsigned short>(floatingImage,
					warpedImage, deformationField, mask, interp, paddingValue,
					dtIndicies, jacMat);
			break;
		case NIFTI_TYPE_INT16:
			reg_resampleImage2<float, short>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_UINT32:
			reg_resampleImage2<float, unsigned int>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_INT32:
			reg_resampleImage2<float, int>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_FLOAT32:
			reg_resampleImage2<float, float>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_FLOAT64:
			reg_resampleImage2<float, double>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		default:
	...
	case NIFTI_TYPE_FLOAT64:
		...
		case NIFTI_TYPE_UINT8:
			...
			break;
		case NIFTI_TYPE_INT8:
			reg_resampleImage2<double, char>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_UINT16:
			...
			break;
		case NIFTI_TYPE_INT16:
			reg_resampleImage2<double, short>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_UINT32:
			reg_resampleImage2<double, unsigned int>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_INT32:
			reg_resampleImage2<double, int>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_FLOAT32:
			reg_resampleImage2<double, float>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		case NIFTI_TYPE_FLOAT64:
			reg_resampleImage2<double, double>(floatingImage, warpedImage,
					deformationField, mask, interp, paddingValue, dtIndicies,
					jacMat);
			break;
		default:
...
void reg_bilinearResampleGradient(nifti_image *floatingImage,
		nifti_image *warpedImage, nifti_image *deformationField,
		float paddingValue) {
	size_t floatingVoxelNumber = (size_t) floatingImage->nx * floatingImage->ny
			* floatingImage->nz;
	size_t warpedVoxelNumber = (size_t) warpedImage->nx * warpedImage->ny
	...
	DTYPE *deformationFieldPtrY = &deformationFieldPtrX[deformationField->nx
...
// Reorientation matrix is assessed in order to remove the rigid component
	mat33 reorient = nifti_mat33_inverse(
			nifti_mat33_polar(reg_mat44_to_mat33(&deformationField->sto_xyz)));
...
// Compute the index in the floating image
			...
			xFloCoord = floating_mm_to_voxel->m[0][0] * defX
					+ floating_mm_to_voxel->m[0][1] * defY
					+ floating_mm_to_voxel->m[0][3];
			yFloCoord = floating_mm_to_voxel->m[1][0] * defX
			...
			for (b = 0; b < 2; ++b) {
				if (anteIntY[b] > -1 && anteIntY[b] < floatingImage->ny) {
					for (a = 0; a < 2; ++a) {
						...
						if (anteIntX[a] > -1
								&& anteIntX[a] < floatingImage->nx) {
							floIndex = anteIntY[b] * floatingImage->nx
									+ anteIntX[a];
...
// Modulate the gradient scalar values
			warpedIntensityX[warpedIndex] = jacMat.m[0][0] * val_x
					+ jacMat.m[0][1] * val_y;
			warpedIntensityY[warpedIndex] = jacMat.m[1][0] * val_x
					+ jacMat.m[1][1] * val_y;
			++warpedIndex;
#ifndef _OPENMP
// Announce the progress via CLI
			if (iProgressStep % progressUnit == 0)
				progressXML(100 * iProgressStep / nProgressSteps,
						"Performing Bilinear Gradient Resampling...");
// Increment the progress counter
			iProgressStep++;
#endif
		} // x
...
void reg_trilinearResampleGradient(nifti_image *floatingImage,
	...
	DTYPE *deformationFieldPtrY = &deformationFieldPtrX[deformationField->nx
			* deformationField->ny * deformationField->nz];
	DTYPE *deformationFieldPtrZ = &deformationFieldPtrY[deformationField->nx
			* deformationField->ny * deformationField->nz];
// Extract the relevant affine matrix
	mat44 *floating_mm_to_voxel = &floatingImage->qto_ijk;
	if (floatingImage->sform_code != 0)
...
// Compute the index in the floating image
				...
				yFloCoord = floating_mm_to_voxel->m[1][0] * defX
						...
						+ floating_mm_to_voxel->m[1][3];
				zFloCoord = floating_mm_to_voxel->m[2][0] * defX
								...
								for (a = 0; a < 2; ++a) {
									...
									if (anteIntX[a] > -1
										...
										floIndex = (anteIntZ[c]
												...
												* floatingImage->nx
												+ anteIntX[a];
										val_x += floatingIntensityX[floIndex]
												* weight[0];
										val_y += floatingIntensityY[floIndex]
												* weight[0];
										val_z += floatingIntensityZ[floIndex]
												* weight[0];
									} // anteIntX not in the floating image space
									else {
										val_x += paddingValue * weight[0];
...
// Get the deformation field index
							defIndex = (anteIntZ[0] * deformationField->ny
									+ anteIntY[0]) * deformationField->nx
									+ anteIntX[0];
// Get the deformation field values
							defX = deformationFieldPtrX[defIndex];
							defY = deformationFieldPtrY[defIndex];
...
// Modulate the gradient scalar values
				warpedIntensityX[warpedIndex] = jacMat.m[0][0] * val_x
						+ jacMat.m[0][1] * val_y + jacMat.m[0][2] * val_z;
				warpedIntensityY[warpedIndex] = jacMat.m[1][0] * val_x
						+ jacMat.m[1][1] * val_y + jacMat.m[1][2] * val_z;
				warpedIntensityZ[warpedIndex] = jacMat.m[2][0] * val_x
						+ jacMat.m[2][1] * val_y + jacMat.m[2][2] * val_z;
				++warpedIndex;
#ifndef _OPENMP
// Announce the progress via CLI
				if (iProgressStep % progressUnit == 0)
					progressXML(100 * iProgressStep / nProgressSteps,
							"Performing Trilinear Gradient Resampling...");
// Increment the progress counter
				iProgressStep++;
#endif
			} // x
...
/* *************************************************************** */
void reg_resampleGradient(nifti_image *floatingImage, nifti_image *warpedImage,
	...
	if (floatingImage->datatype != warpedImage->datatype
		...
		fprintf(stderr,
				"[NiftyReg ERROR] reg_resampleGradient - Input images are expected to have the same type\n");
		reg_exit(1);
	}
	switch (floatingImage->datatype) {
	case NIFTI_TYPE_FLOAT32:
		if (warpedImage->nz > 1) {
			reg_trilinearResampleGradient<float>(floatingImage, warpedImage,
					deformationField, paddingValue);
		} else {
			reg_bilinearResampleGradient<float>(floatingImage, warpedImage,
					deformationField, paddingValue);
		}
		break;
	case NIFTI_TYPE_FLOAT64:
		if (warpedImage->nz > 1) {
			reg_trilinearResampleGradient<double>(floatingImage, warpedImage,
					deformationField, paddingValue);
		} else {
			reg_bilinearResampleGradient<double>(floatingImage, warpedImage,
					deformationField, paddingValue);
		}
		break;
	default:
		fprintf(stderr,
				"[NiftyReg ERROR] reg_resampleGradient - Only single and double floating precision are supported\n");
		reg_exit(1)
		;
	}
}
...
void TrilinearImageGradient(nifti_image *floatingImage,
		nifti_image *deformationField, nifti_image *warpedGradientImage,
...
#else
	size_t index;
	size_t referenceVoxelNumber = (size_t) warpedGradientImage->nx
			* warpedGradientImage->ny * warpedGradientImage->nz;
	size_t floatingVoxelNumber = (size_t) floatingImage->nx * floatingImage->ny
			* floatingImage->nz;
#endif
	FloatingTYPE *floatingIntensityPtr =
			static_cast<FloatingTYPE *>(floatingImage->data);
	GradientTYPE *warpedGradientImagePtr =
			static_cast<GradientTYPE *>(warpedGradientImage->data);
	FieldTYPE *deformationFieldPtrX =
			static_cast<FieldTYPE *>(deformationField->data);
	FieldTYPE *deformationFieldPtrY =
			&deformationFieldPtrX[referenceVoxelNumber];
	FieldTYPE *deformationFieldPtrZ =
			&deformationFieldPtrY[referenceVoxelNumber];
	int *maskPtr = &mask[0];
	mat44 *floatingIJKMatrix;
	if (floatingImage->sform_code > 0)
		floatingIJKMatrix = &(floatingImage->sto_ijk);
...
// Compute the resolution of the progress bar
	unsigned long iProgressStep = 1;
	unsigned long nProgressSteps = warpedGradientImage->nt
			* referenceVoxelNumber;
	unsigned long progressUnit = (unsigned long) ceil(
			(float) nProgressSteps / 100.0f);
#endif
// Iteration over the different volume along the 4th axis
	for (int t = 0; t < warpedGradientImage->nt; t++) {
#ifndef NDEBUG
		printf(
				"[NiftyReg DEBUG] 3D linear gradient computation of volume number %i\n",
				t);
#endif
		GradientTYPE *warpedGradientPtrX = &warpedGradientImagePtr[t * 3
				* referenceVoxelNumber];
		GradientTYPE *warpedGradientPtrY =
				&warpedGradientPtrX[referenceVoxelNumber];
		GradientTYPE *warpedGradientPtrZ =
				&warpedGradientPtrY[referenceVoxelNumber];
		FloatingTYPE *floatingIntensity = &floatingIntensityPtr[t
				* floatingVoxelNumber];
		...
		FieldTYPE xxTempNewValue, yyTempNewValue, zzTempNewValue, xTempNewValue,
				yTempNewValue;
		FloatingTYPE *zPointer, *xyzPointer;
#if defined (NDEBUG) && defined (_OPENMP)
#pragma omp parallel for default(none) \
...
// The padding value is used for interpolation if it is different from NaN
								...
								if (Y > -1 && Y < floatingImage->ny) {
									...
									for (a = 0; a < 2; a++) {
										...
										if (X > -1 && X < floatingImage->nx) {
											...
											xTempNewValue += paddingValue
													* deriv[a];
											yTempNewValue += paddingValue
													* xBasis[a];
										}
										xyzPointer++;
									} // end a
				...
				else if (previous[0] >= 0.f
						&& previous[0] < (floatingImage->nx - 1)
						&& previous[1] >= 0.f
						&& previous[1] < (floatingImage->ny - 1)
						&& previous[2] >= 0.f
						&& previous[2] < (floatingImage->nz - 1)) {
					for (c = 0; c < 2; c++) {
						Z = previous[2] + c;
						zPointer = &floatingIntensity[Z * floatingImage->nx
								* floatingImage->ny];
...
// Announce the progress via CLI
			if (iProgressStep % progressUnit == 0)
				progressXML(100 * iProgressStep / nProgressSteps,
						"Performing Trilinear Gradient Computation...");
// Increment the progress counter
			iProgressStep++;
#endif
		}
...
#ifndef NDEBUG
		printf(
				"[NiftyReg DEBUG] 2D linear gradient computation of volume number %i\n",
				t);
#endif
		GradientTYPE *warpedGradientPtrX = &warpedGradientImagePtr[2 * t
				* referenceVoxelNumber];
		GradientTYPE *warpedGradientPtrY =
				&warpedGradientPtrX[referenceVoxelNumber];
		FloatingTYPE *floatingIntensity = &floatingIntensityPtr[t
				* floatingVoxelNumber];
		FieldTYPE position[3], xBasis[2], yBasis[2], relative, world[2],
				grad[2];
		FieldTYPE deriv[2];
		deriv[0] = -1;
		deriv[1] = 1;
...
// Announce the progress via CLI
			if (iProgressStep % progressUnit == 0)
				progressXML(100 * iProgressStep / nProgressSteps,
						"Performing Bilinear Gradient Computation...");
// Increment the progress counter
			iProgressStep++;
#endif
		}
...
// Iteration over the different volume along the 4th axis
	for (int t = 0; t < warpedGradientImage->nt; t++) {
#ifndef NDEBUG
		printf(
				"[NiftyReg DEBUG] 3D cubic spline gradient computation of volume number %i\n",
				t);
#endif
		GradientTYPE *warpedGradientPtrX = &warpedGradientImagePtr[3 * t
				* referenceVoxelNumber];
		GradientTYPE *warpedGradientPtrY =
				&warpedGradientPtrX[referenceVoxelNumber];
		GradientTYPE *warpedGradientPtrZ =
		...
		double xBasis[4], yBasis[4], zBasis[4], xDeriv[4], yDeriv[4], zDeriv[4],
				relative;
...
// Announce the progress via CLI
			if (iProgressStep % progressUnit == 0)
				progressXML(100 * iProgressStep / nProgressSteps,
						"Performing 3D Cubic Spline Gradient Computation...");
// Increment the progress counter
			iProgressStep++;
#endif
		}
...
#ifndef NDEBUG
		printf(
				"[NiftyReg DEBUG] 2D cubic spline gradient computation of volume number %i\n",
				t);
#endif
		GradientTYPE *warpedGradientPtrX = &warpedGradientImagePtr[t * 2
				* referenceVoxelNumber];
		GradientTYPE *warpedGradientPtrY =
				&warpedGradientPtrX[referenceVoxelNumber];
		FloatingTYPE *floatingIntensity = &floatingIntensityPtr[t
...
// Announce the progress via CLI
			if (iProgressStep % progressUnit == 0)
				progressXML(100 * iProgressStep / nProgressSteps,
						"Performing 2D Cubic Spline Gradient Computation...");
// Increment the progress counter
			iProgressStep++;
#endif
		}
...
void reg_getImageGradient3(nifti_image *floatingImage,
		nifti_image *warpedGradientImage, nifti_image *deformationField,
		int *mask, int interp, float paddingValue, int *dtIndicies,
		mat33 *jacMat, nifti_image *warpedImage = NULL) {
// The floating image data is copied in case one deal with DTI
	void *originalFloatingData = NULL;
// The DTI are logged
	reg_dti_resampling_preprocessing<FloatingTYPE>(floatingImage,
	...
	if (interp == 3) {
		if (deformationField->nz > 1) {
			CubicSplineImageGradient3D<FloatingTYPE, GradientTYPE, FieldTYPE>(
					floatingImage, deformationField, warpedGradientImage, mask,
					paddingValue);
		} else {
			CubicSplineImageGradient2D<FloatingTYPE, GradientTYPE, FieldTYPE>(
					floatingImage, deformationField, warpedGradientImage, mask);
		}
	} else // trilinear interpolation [ by default ]
	{
		if (deformationField->nz > 1) {
			TrilinearImageGradient<FloatingTYPE, GradientTYPE, FieldTYPE>(
					floatingImage, deformationField, warpedGradientImage, mask,
					paddingValue);
		} else {
			BilinearImageGradient<FloatingTYPE, GradientTYPE, FieldTYPE>(
					floatingImage, deformationField, warpedGradientImage, mask,
					paddingValue);
		}
	}
...
// The interpolated tensors are reoriented and exponentiated
	reg_dti_resampling_postprocessing<FloatingTYPE>(warpedGradientImage, mask,
			jacMat, dtIndicies, warpedImage);
}
/* *************************************************************** */
template<class FieldTYPE, class FloatingTYPE>
void reg_getImageGradient2(nifti_image *floatingImage,
		nifti_image *warpedGradientImage, nifti_image *deformationField,
		int *mask, int interp, float paddingValue, int *dtIndicies,
		mat33 *jacMat, nifti_image *warpedImage) {
	switch (warpedGradientImage->datatype) {
	case NIFTI_TYPE_FLOAT32:
		reg_getImageGradient3<FieldTYPE, FloatingTYPE, float>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	case NIFTI_TYPE_FLOAT64:
		reg_getImageGradient3<FieldTYPE, FloatingTYPE, double>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	default:
		printf(
				"[NiftyReg ERROR] reg_getVoxelBasedNMIGradientUsingPW\tThe warped image data type is not supported\n");
		return;
	}
}
/* *************************************************************** */
...
void reg_getImageGradient1(nifti_image *floatingImage,
	...
	switch (floatingImage->datatype) {
	case NIFTI_TYPE_UINT8:
		reg_getImageGradient2<FieldTYPE, unsigned char>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	case NIFTI_TYPE_INT8:
		reg_getImageGradient2<FieldTYPE, char>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	case NIFTI_TYPE_UINT16:
		reg_getImageGradient2<FieldTYPE, unsigned short>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	case NIFTI_TYPE_INT16:
		reg_getImageGradient2<FieldTYPE, short>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	case NIFTI_TYPE_UINT32:
		reg_getImageGradient2<FieldTYPE, unsigned int>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	case NIFTI_TYPE_INT32:
		reg_getImageGradient2<FieldTYPE, int>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	case NIFTI_TYPE_FLOAT32:
		reg_getImageGradient2<FieldTYPE, float>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	case NIFTI_TYPE_FLOAT64:
		reg_getImageGradient2<FieldTYPE, double>(floatingImage,
				warpedGradientImage, deformationField, mask, interp,
				paddingValue, dtIndicies, jacMat, warpedImage);
		break;
	default:
...
// voxels in the backgreg_round are set to -1 so 0 will do the job here
		mask = (int *) calloc(
				deformationField->nx * deformationField->ny
						* deformationField->nz, sizeof(int));
		MrPropreRule = true;
	}
// Check if the dimension are correct
	if (floatingImage->nt != warpedGradientImage->nt) {
		printf(
				"[NiftyReg ERROR] reg_getImageGradient\tThe floating and warped images have different dimension along the time axis\n");
		printf(
				"[NiftyReg ERROR] reg_getImageGradient\tNothing has been done\n");
		return;
	}
// Define the DTI indices if required
	int dtIndicies[6];
	...
	switch (deformationField->datatype) {
	case NIFTI_TYPE_FLOAT32:
		reg_getImageGradient1<float>(floatingImage, warpedGradientImage,
				deformationField, mask, interp, paddingValue, dtIndicies,
				jacMat, warpedImage);
		break;
	case NIFTI_TYPE_FLOAT64:
		reg_getImageGradient1<double>(floatingImage, warpedGradientImage,
				deformationField, mask, interp, paddingValue, dtIndicies,
				jacMat, warpedImage);
		break;
	default:
		printf(
				"[NiftyReg ERROR] reg_getImageGradient\tDeformation field pixel type unsupported.\n");
		break;
	}
	if (MrPropreRule == true)
		free(mask);
}
/* *************************************************************** */
/* *************************************************************** */
...
// Define the size of the new image
	...
	for (size_t i = 1; i < 4; ++i) {
		if (i < static_cast<size_t>(img->dim[0] + 1))
			newDim[i] = (int) ceilf(
					img->dim[i] * img->pixdim[i] / smallestPixDim);
	}
...
// Update the qform matrix
	...
	newImg->qto_xyz = nifti_quatern_to_mat44(newImg->quatern_b,
			newImg->quatern_c, newImg->quatern_d, newImg->qoffset_x,
			newImg->qoffset_y, newImg->qoffset_z, smallestPixDim,
			smallestPixDim, smallestPixDim, newImg->qfac);
...
// Compute the new sform
		...
		newImg->sto_xyz.m[0][3] = img->sto_xyz.m[0][3] + smallestPixDim / 2.f
				- img->dx / 2.f;
		newImg->sto_xyz.m[1][3] = img->sto_xyz.m[1][3] + smallestPixDim / 2.f
				- img->dy / 2.f;
		newImg->sto_xyz.m[2][3] = img->sto_xyz.m[2][3] + smallestPixDim / 2.f
				- img->dz / 2.f;
...
// Create a deformation field
	...
	def->nvox = (size_t) def->nx * (size_t) def->ny * (size_t) def->nz
			* (size_t) def->nt * (size_t) def->nu;

...
__global__ void reg_resampleImage2D_kernel(float *resultArray)
{
	...
	if(tid<c_ActiveVoxelNumber){
		...
		float2 voxelPosition;
		float4 matrix = tex1Dfetch(sourceMatrixTexture,0);
		voxelPosition.x =
				matrix.x*realPosition.x +
				matrix.y*realPosition.y +
				matrix.w;
	...
	if(tid<c_ActiveVoxelNumber){
		...
		for(short b=0; b<2; b++){
			...
			for(short a=0; a<2; a++){
				...
				if(0.f<=relativePosition.x && relativePosition.x<=1.f &&
				   0.f<=relativePosition.y && relativePosition.y<=1.f)
					intensity=tex3D(sourceTexture,
									relativePosition.x,
									relativePosition.y,
									0.5f);
		...
		for(short c=0; c<2; c++){
			...
			for(short b=0; b<2; b++){
				...
				for(short a=0; a<2; a++){
					...
					if(0.f<=relativePosition.x && relativePosition.x<=1.f &&
					   0.f<=relativePosition.y && relativePosition.y<=1.f &&
					   0.f<=relativePosition.z && relativePosition.z<=1.f)
						intensity=tex3D(sourceTexture,
										relativePosition.x,
										relativePosition.y,
										relativePosition.z);

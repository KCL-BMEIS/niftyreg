...
public:
  ...
  void setSaveMoreOutput(bool saveMore);
...
template <class T>
void reg_f3d2_ipopt<T>::setSaveMoreOutput(bool saveMore) {
    this->saveMoreOutput = saveMore;
}
...
//        nifti_image_free(jacobianDeterminantArray);
    ...
    if (this->saveMoreOutput) {
        ...
        if (outputWarpedImage[1] != NULL) {
            fileName = stringFormat("%s/output_warped_ref_level%d.nii.gz", this->saveDir.c_str(),
                                    this->currentLevel + 1);
...
//        // compute and save images with an Euler integration
...
//        fileName = stringFormat("%s/output_warped_euler_flo_level%d.nii.gz", this->saveDir.c_str(),
//                                this->currentLevel + 1);
...
//            fileName = stringFormat("%s/output_warped_euler_ref_level%d.nii.gz", this->saveDir.c_str(),
//                                    this->currentLevel + 1);
...
//        fileName = stringFormat("%s/output_abs_error_euler_level%d.nii.gz", this->saveDir.c_str(),
//                                this->currentLevel + 1);
//        reg_io_WriteImageFile(outputWarpedImageEuler[0], fileName.c_str());
//        // free allocated memory

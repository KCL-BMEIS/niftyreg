...
void reg_linear_spline_getDeformationField3D(nifti_image *splineControlPoint,
    ...
    if (composition) { // Composition of deformation fields
        ...
        const mat44 *gridVoxelToReal = splineControlPoint->sform_code > 0
            ? &splineControlPoint->sto_xyz : &splineControlPoint->qto_xyz;
            ...
            for (y = 0; y < deformationField->ny; y++) {
                for (x = 0; x < deformationField->nx; x++) {
                    if (mask[index] > -1) {
                        ...
                        real[2] = 0;
                        for (c = 0; c < 2; c++) {
                            const int zInd = zPre + c;
                            const bool inZ = -1 < zInd && zInd < splineControlPoint->nz;
                            for (b = 0; b < 2; b++) {
                                const int yInd = yPre + b;
                                const bool inY = -1 < yInd && yInd < splineControlPoint->ny;
                                for (a = 0; a < 2; a++) {
                                    const int xInd = xPre + a;
                                    DataType xValue, yValue, zValue;
                                    if (-1 < xInd && xInd < splineControlPoint->nx && inY && inZ) {
                                        // Uses the grid value if the node is in its space
                                        coord = (zInd * splineControlPoint->ny + yInd) * splineControlPoint->nx + xInd;
                                        xValue = controlPointPtrX[coord];
                                        yValue = controlPointPtrY[coord];
                                        zValue = controlPointPtrZ[coord];
                                    } else {
                                        // Uses a sliding effect
                                        get_SlidedValues<DataType>(xValue,
                                                                   yValue,
                                                                   zValue,
                                                                   xInd,
                                                                   yInd,
                                    ...
                                    real[0] += xValue * tempValue;
                                    real[1] += yValue * tempValue;
                                    real[2] += zValue * tempValue;
                                }
                            }
                        }
        ...
        for (y = 0; y < deformationField->ny; y++) {
            ...
            for (x = 0; x < deformationField->nx; x++) {
                if (mask[index] > -1) {
                    ...
                    // No bounds check on the position, matching reg_cubic_spline_getDeformationField3D:
                    // taps falling outside the grid are supplied by get_GridValues, which extrapolates
                    // through get_SlidedValues, so every voxel is composed wherever it lands. Note that
                    // xReal/yReal are only reassigned below while the write that follows is
                    // unconditional, so any guard placed here would write the position back unchanged
                    // rather than skipping the voxel.
                    {
...
void reg_defField_getDeformationFieldFromFlowField(nifti_image *flowField,
    ...
    if (updateStepNumber || flowField->intent_p2 == 0) {
        ...
        float extrema, temp;
        std::tie(extrema, temp) = NiftiImage(flowField).data().minmax();
    ...
    float scalingValue = static_cast<float>(std::pow(2, squaringNumber));
    if (flowField->intent_p2 < 0)

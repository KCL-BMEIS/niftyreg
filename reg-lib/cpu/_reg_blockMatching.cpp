...
double get_square_distance3D(float * first_point3D, float * second_point3D) {
	return sqrt(
			(first_point3D[0] - second_point3D[0])
					* (first_point3D[0] - second_point3D[0])
					+ (first_point3D[1] - second_point3D[1])
							* (first_point3D[1] - second_point3D[1])
					+ (first_point3D[2] - second_point3D[2])
							* (first_point3D[2] - second_point3D[2]));
}
/* *************************************************************** */
double get_square_distance2D(float * first_point2D, float * second_point2D) {
	return sqrt(
			(first_point2D[0] - second_point2D[0])
					* (first_point2D[0] - second_point2D[0])
					+ (first_point2D[1] - second_point2D[1])
							* (first_point2D[1] - second_point2D[1]));
}
/* *************************************************************** */
/* *************************************************************** */
template<class DTYPE>
void _reg_set_active_blocks(nifti_image *targetImage,
		_reg_blockMatchingParam *params, int *mask, bool runningOnGPU) {
	const size_t totalBlockNumber = params->blockNumber[0]
			* params->blockNumber[1] * params->blockNumber[2];
...
// Version using 2D blocks
			...
			for (int i = 0; i < params->blockNumber[0]; i++) {
				...
				for (int y = j * BLOCK_WIDTH; y < (j + 1) * BLOCK_WIDTH; y++) {
					if (y < targetImage->ny) {
						...
						for (int x = i * BLOCK_WIDTH; x < (i + 1) * BLOCK_WIDTH;
								x++) {
							if (x < targetImage->nx) {
								targetValues[coord] = *targetPtrXY;
								if (targetValues[coord] == targetValues[coord]
										&& targetValues[coord] != 0.
										&& *maskPtrXY > -1) {
	...
	params->activeBlockNumber =
			params->activeBlockNumber
					< ((int) totalBlockNumber - unusableBlock) ?
					params->activeBlockNumber :
					(totalBlockNumber - unusableBlock);
...
void initialise_block_matching_method(nifti_image * target,
		_reg_blockMatchingParam *params, int percentToKeep_block,
		int percentToKeep_opt, int stepSize_block, int *mask,
		bool runningOnGPU) {
	...
	params->blockNumber[0] =
			(int) reg_ceil((float)target->nx / (float)BLOCK_WIDTH);
	params->blockNumber[1] =
			(int) reg_ceil((float)target->ny / (float)BLOCK_WIDTH);
	if (target->nz > 1)
		params->blockNumber[2] =
				(int) reg_ceil((float)target->nz / (float)BLOCK_WIDTH);
	else
		params->blockNumber[2] = 1;
	params->stepSize = stepSize_block;
	params->percent_to_keep = percentToKeep_opt;
	params->activeBlockNumber = params->blockNumber[0] * params->blockNumber[1]
			* params->blockNumber[2] * percentToKeep_block / 100;
	params->activeBlock = (int *) malloc(
			params->blockNumber[0] * params->blockNumber[1]
					* params->blockNumber[2] * sizeof(int));
	...
	default:
		fprintf(stderr,
				"[NiftyReg ERROR] initialise_block_matching_method\tThe target image data type is not supported\n");
		reg_exit(1)
		;
	}
...
#endif
	if (target->nz > 1) {
		params->targetPosition = (float *) malloc(
				params->activeBlockNumber * 3 * sizeof(float));
		params->resultPosition = (float *) malloc(
				params->activeBlockNumber * 3 * sizeof(float));
	} else {
		params->targetPosition = (float *) malloc(
				params->activeBlockNumber * 2 * sizeof(float));
		params->resultPosition = (float *) malloc(
				params->activeBlockNumber * 2 * sizeof(float));
	}
#ifndef NDEBUG
...
void block_matching_method2D(nifti_image * target, nifti_image * result,
	...
	TargetImageType *targetValues = (TargetImageType *) malloc(
			BLOCK_2D_SIZE * sizeof(TargetImageType));
	bool *targetOverlap = (bool *) malloc(BLOCK_2D_SIZE * sizeof(bool));
	ResultImageType *resultValues = (ResultImageType *) malloc(
			BLOCK_2D_SIZE * sizeof(ResultImageType));
			...
			if (params->activeBlock[blockIndex] > -1) {
				...
				for (int y = targetIndex_start_y; y < targetIndex_end_y; y++) {
					if (-1 < y && y < target->ny) {
						...
						for (int x = targetIndex_start_x; x < targetIndex_end_x;
								x++) {
							if (-1 < x && x < target->nx) {
...
// iteration over the result blocks
				for (int m = -OVERLAP_SIZE; m <= OVERLAP_SIZE;
					...
					for (int l = -OVERLAP_SIZE; l <= OVERLAP_SIZE;
						...
						for (int y = resultIndex_start_y; y < resultIndex_end_y;
								y++) {
							if (-1 < y && y < result->ny) {
								index = y * result->nx + resultIndex_start_x;
								ResultImageType *resultPtr_XY =
										&resultPtr[index];
								int *maskPtr_XY = &mask[index];
								for (int x = resultIndex_start_x;
										x < resultIndex_end_x; x++) {
									if (-1 < x && x < result->nx) {
						...
						if (voxelNumber > BLOCK_2D_SIZE / 2) {
							...
							for (int a = 0; a < BLOCK_2D_SIZE; a++) {
								if (targetOverlap[a] && resultOverlap[a]) {
									PrecisionTYPE targetTemp =
											(PrecisionTYPE) (targetValues[a]
													- targetMean);
									PrecisionTYPE resultTemp =
											(PrecisionTYPE) (resultValues[a]
													- resultMean);
							...
							localCC = fabs(
									localCC / sqrt(targetVar * resultVar));
							if (localCC > bestCC) {
								bestCC = localCC;
				...
				if (bestDisplacement[0] == bestDisplacement[0]) {
					...
					reg_mat44_mul(targetMatrix_xyz, targetPosition_temp,
							tempPosition);
					params->targetPosition[activeBlockIndex] = tempPosition[0];
					params->targetPosition[activeBlockIndex + 1] =
							tempPosition[1];
					reg_mat44_mul(targetMatrix_xyz, bestDisplacement,
							tempPosition);
					params->resultPosition[activeBlockIndex] = tempPosition[0];
					params->resultPosition[activeBlockIndex + 1] =
							tempPosition[1];
					activeBlockIndex += 2;
...
void block_matching_method3D(nifti_image * target, nifti_image * result,
		_reg_blockMatchingParam *params, int *mask) {
...
#endif
	float *temp_target_position = (float *) malloc(
			3 * params->activeBlockNumber * sizeof(float));
	float *temp_result_position = (float *) malloc(
			3 * params->activeBlockNumber * sizeof(float));
				...
				if (params->activeBlock[blockIndex] > -1) {
					...
					for (z = targetIndex_start_z; z < targetIndex_end_z; z++) {
						if (-1 < z && z < target->nz) {
							...
							for (y = targetIndex_start_y; y < targetIndex_end_y;
									y++) {
								if (-1 < y && y < target->ny) {
									index = y * target->nx
											+ targetIndex_start_x;
									...
									for (x = targetIndex_start_x;
											x < targetIndex_end_x; x++) {
										if (-1 < x && x < target->nx) {
											value = *targetPtr_XYZ;
											if (value == value
													&& *maskPtr_XYZ > -1) {
												targetValues[tid][targetIndex] =
														value;
												targetOverlap[tid][targetIndex] =
														1;
											}
										}
										targetPtr_XYZ++;
...
// iteration over the result blocks
					for (n = -OVERLAP_SIZE; n <= OVERLAP_SIZE;
							n += params->stepSize) {
						resultIndex_start_z = targetIndex_start_z + n;
						resultIndex_end_z = resultIndex_start_z + BLOCK_WIDTH;
						for (m = -OVERLAP_SIZE; m <= OVERLAP_SIZE;
								m += params->stepSize) {
							resultIndex_start_y = targetIndex_start_y + m;
							resultIndex_end_y = resultIndex_start_y
									+ BLOCK_WIDTH;
							for (l = -OVERLAP_SIZE; l <= OVERLAP_SIZE; l +=
								...
								for (z = resultIndex_start_z;
									...
									if (-1 < z && z < result->nz) {
										...
										for (y = resultIndex_start_y;
												y < resultIndex_end_y; y++) {
											if (-1 < y && y < result->ny) {
												index = y * result->nx
														+ resultIndex_start_x;
												resultPtr_XYZ =
												...
												for (x = resultIndex_start_x;
													...
													if (-1 < x
														...
														if (value == value
																&& *maskPtr_XYZ
																		> -1) {
															resultValues[tid][resultIndex] =
																	value;
															resultOverlap[tid][resultIndex] =
																	1;
														}
													}
													resultPtr_XYZ++;
								...
								if (voxelNumber > BLOCK_SIZE / 2) {
									...
									for (int a = 0; a < BLOCK_SIZE; a++) {
										if (targetOverlap[tid][a]
												&& resultOverlap[tid][a]) {
											targetTemp = (targetValues[tid][a]
													- targetMean);
											resultTemp = (resultValues[tid][a]
													- resultMean);
											targetVar += (targetTemp)
													* (targetTemp);
											resultVar += (resultTemp)
													* (resultTemp);
											localCC += (targetTemp)
													* (resultTemp);
...
void block_matching_method(nifti_image * target, nifti_image * result,
		_reg_blockMatchingParam *params, int *mask) {
	if (target->datatype != result->datatype) {
		reg_print_fct_error("block_matching_method");
		reg_print_msg_error(
				"Both input images are expected to be of the same type");
	}
	if (target->nz == 1) {
		switch (target->datatype) {
		case NIFTI_TYPE_FLOAT64:
			block_matching_method2D<double, double, double>(target, result,
					params, mask);
			break;
		case NIFTI_TYPE_FLOAT32:
			block_matching_method2D<float, float, float>(target, result, params,
					mask);
			break;
		default:
			reg_print_fct_error("block_matching_method")
...
void apply_affine2D(mat44 * mat, float *pt, float *result) {
	result[0] = static_cast<float>(static_cast<double>(pt[0])
			* static_cast<double>(mat->m[0][0])
			+ static_cast<double>(pt[1]) * static_cast<double>(mat->m[0][1])
			+ static_cast<double>(mat->m[0][3]));
	result[1] = static_cast<float>(static_cast<double>(pt[0])
			* static_cast<double>(mat->m[1][0])
...
void mul_matrices(float ** a, float ** b, int ar, int ac, int bc, float ** r,
		bool transposeB) {
	if (transposeB) {
		for (int i = 0; i < ar; ++i) {
			for (int j = 0; j < bc; ++j) {
				r[i][j] = 0.0f;
				for (int k = 0; k < ac; ++k) {
					r[i][j] += static_cast<float>(static_cast<double>(a[i][k])
							* static_cast<double>(b[j][k]));
				}
			}
		}
	} else {
...
void mul_matvec(float ** a, int ar, int ac, float * b, float * r) {
	for (int i = 0; i < ar; ++i) {
		r[i] = 0;
		for (int k = 0; k < ac; ++k) {
			r[i] += static_cast<float>(static_cast<double>(a[i][k])
...
void estimate_affine_transformation2D(std::vector<_reg_sorted_point2D> &points,
		mat44 * transformation, float ** A, float * w, float ** v, float ** r,
		float * b) {
...
// we need at least 4 points. Assuming we have that here.
	...
	for (unsigned k = 0; k < points.size(); ++k) {
		...
		A[c][3] = A[c][4] = A[c][5] = A[c][6] = A[c][7] = A[c][8] = A[c][10] =
				A[c][11] = 0.0f;
		A[c][9] = 1.0f;
		A[c + 1][3] = points[k].target[0];
		A[c + 1][4] = points[k].target[1];
		A[c + 1][5] = points[k].target[2];
		A[c + 1][0] = A[c + 1][1] = A[c + 1][2] = A[c + 1][6] = A[c + 1][7] =
				A[c + 1][8] = A[c + 1][9] = A[c + 1][11] = 0.0f;
		A[c + 1][10] = 1.0f;
		A[c + 2][6] = points[k].target[0];
		A[c + 2][7] = points[k].target[1];
		A[c + 2][8] = points[k].target[2];
		A[c + 2][0] = A[c + 2][1] = A[c + 2][2] = A[c + 2][3] = A[c + 2][4] =
				A[c + 2][5] = A[c + 2][9] = A[c + 2][10] = 0.0f;
		A[c + 2][11] = 1.0f;
	}
...
// Simply scale each column by the corresponding singular value
	for (unsigned k = 0; k < 12; ++k) {
		for (unsigned j = 0; j < 12; ++j) {
			v[j][k] = static_cast<float>(static_cast<double>(v[j][k])
...
// The LS in the iterations is done on subsample of the input data
	...
	const unsigned long num_to_keep = (unsigned long) (num_points
			* (params->percent_to_keep / 100.0f));
...
// Transform the points in the target
		for (unsigned j = 0; j < num_points * 2; j += 2) {
			apply_affine2D(final, &(params->targetPosition[j]),
					&newResultPosition[j]);
		}
		...
		for (unsigned j = 0; j < num_points * 2; j += 2) {
			...
			queue.insert(
					std::pair<double, _reg_sorted_point2D>(distance,
							_reg_sorted_point2D(&(params->targetPosition[j]),
		...
		i = 0;
		top_points.clear();
		for (std::multimap<double, _reg_sorted_point2D>::iterator it =
				queue.begin(); it != queue.end(); ++it, ++i) {
			if (i >= num_to_keep)
				break;
...
// Transform the points in the target
		for (unsigned j = 0; j < num_points * 3; j += 3) {
			reg_mat44_mul(final, &(params->targetPosition[j]),
					&newResultPosition[j]);
		}
		queue = std::multimap<double, _reg_sorted_point3D>();
		for (unsigned j = 0; j < num_points * 3; j += 3) {
			distance = get_square_distance3D(&newResultPosition[j],
					&(params->resultPosition[j]));
			queue.insert(
					std::pair<double, _reg_sorted_point3D>(distance,
							_reg_sorted_point3D(&(params->targetPosition[j]),
									&(params->resultPosition[j]), distance)));
		}
		distance = 0.0;
		i = 0;
		top_points.clear();
		for (std::multimap<double, _reg_sorted_point3D>::iterator it =
				queue.begin(); it != queue.end(); ++it, ++i) {
			if (i >= num_to_keep)
				break;
			top_points.push_back((*it).second);
...
// Calculate the translation
	float t[2];
	t[0] = centroid_result[0]
			- (r[0][0] * centroid_target[0] + r[0][1] * centroid_target[1]);
	t[1] = centroid_result[1]
			- (r[1][0] * centroid_target[0] + r[1][1] * centroid_target[1]);
...
// Calculate the translation
	...
	t[2] = centroid_result[2]
			- (r[2][0] * centroid_target[0] + r[2][1] * centroid_target[1]
...
// Set the current transformation to identity
	...
	unsigned long num_to_keep = (unsigned long) (num_points
			* (params->percent_to_keep / 100.0f));
	float * newResultPosition = new float[num_points * 2];
	mat44 lastTransformation;
	memset(&lastTransformation, 0, sizeof(mat44));
...
void optimize(_reg_blockMatchingParam *params, mat44 *transformation_matrix,
		bool affine) {
...
// mat44 inverseMatrix = nifti_mat44_inverse(*transformation_matrix);
	if (params->blockNumber[2] == 1) // 2D images
		...
		float in[2];
		float out[2];
		for (size_t i = 0; i < static_cast<size_t>(params->activeBlockNumber);
				++i) {
			size_t index = 2 * i;

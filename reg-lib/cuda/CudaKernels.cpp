		...
		for (unsigned long j = 0; j < refParams->definedActiveBlock; j++) {
			...
			if (targetSum[0] <= threshold && targetSum[1] <= threshold && targetSum[2] <= threshold) {
				...
				if (resultSum[0] > 0.000001f || resultSum[1] > 0.000001f || resultSum[2] > 0.000001f) {
					...
					printf("i: %lu | j: %lu | target: (%f-%f-%f) | (dif: %f-%f-%f) | (cpu: %f, %f, %f) | (ref: %f, %f, %f)\n", i, j, out[0], out[1], out[2], res[0], res[1], res[2], cpuResultPt[0], cpuResultPt[1], cpuResultPt[2], cudaResultPt[0], cudaResultPt[1], cudaResultPt[2]);
					count2++;
				}
			}
		}

...
__global__ void blockMatchingKernel2(float *resultPosition, float *targetPosition, int* mask, float* targetMatrix_xyz, unsigned int* definedBlock, uint3 c_ImageSize, const int numOverlappedBlocks, const unsigned int stepSize) {
...
//	bool predicate = xBaseImage == 16 && yBaseImage == 24 && zBaseImage == 24;
	...
	if (currentBlockIndex > -1) {
		...
		float bestDisplacement[3] = { nanf("sNaN"), 0.0f, 0.0f };
		float bestCC = numOverlappedBlocks>1?0.9f:0.0f;

		//populate shared memory with resultImageArray's values
		for (int n = -1*numOverlappedBlocks; n <= numOverlappedBlocks; n += 1) {
			for (int m = -1*numOverlappedBlocks; m <= numOverlappedBlocks; m += 1) {
				for (int l = -1*numOverlappedBlocks; l <= numOverlappedBlocks; l += 1) {
					const int x = l * 4 + idx;
					const int y = m * 4 + idy;
					const int z = n * 4 + idz;

					const unsigned int sIdx = (z + numOverlappedBlocks*4)* (numOverlappedBlocks*2 +1)*4 * (numOverlappedBlocks*2 +1)*4  + (y + numOverlappedBlocks*4) * (numOverlappedBlocks*2 +1)*4  + (x + numOverlappedBlocks*4);
		...
		if (targetBlockSize > 32) {
			...
			// iteration over the result blocks (block matching part)
			for (unsigned int n = 1; n < numOverlappedBlocks*2*4; n += stepSize) {
				for (unsigned int m = 1; m < numOverlappedBlocks*2*4; m += stepSize) {
					for (unsigned int l = 1; l < numOverlappedBlocks*2*4; l += stepSize) {

						const unsigned int sIdxIn = (idz + n) * (numOverlappedBlocks*2 +1)*4 * (numOverlappedBlocks*2 +1)*4 + (idy + m) * (numOverlappedBlocks*2 +1)*4  + idx + l;
						const float rResultValue = sResultValues[sIdxIn];
						const bool overlap = isfinite(rResultValue) && finiteTargetIntensity;
						const unsigned int blockSize = __syncthreads_count(overlap);
...
//							if (tid == 0 && bestCC != 0.981295f && (localCC > bestCC )) {
							if (tid == 0 && localCC > bestCC ) {
								bestCC = localCC;
								bestDisplacement[0] = l - numOverlappedBlocks*4.0f;
								bestDisplacement[1] = m - numOverlappedBlocks*4.0f;
								bestDisplacement[2] = n - numOverlappedBlocks*4.0f;
							}
							/*if (predicate && tid==0 )
								printf("C|%d-%d-%d|%f-%f-%f\n", l-4, m-4, n-4, bestDisplacement[0], bestDisplacement[1], bestDisplacement[2]);*/
						}

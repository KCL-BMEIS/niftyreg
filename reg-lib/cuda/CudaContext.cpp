...
void CudaContext::PickCard(unsigned deviceId = 999) {
    // Release any previously created context before making a new one (only hit when actually
    ...
    // Select the target device: the requested id if it is valid, otherwise the highest-Gflops card
    // (the auto-pick path, code from cutGetMaxGflopsDeviceId()).
    ...
    // Create the context on the selected device and configure it (shared by both selection paths).
    NR_CUDA_SAFE_CALL(cudaSetDevice(cudaIdx));
    NR_CUDA_SAFE_CALL(Cuda::CtxCreate(&cudaContext, CU_CTX_SCHED_SPIN, cudaIdx));
    NR_CUDA_SAFE_CALL(cudaGetDeviceProperties(&deviceProp, cudaIdx));
    ...
    NR_DEBUG("The card clock rate is " << Cuda::GetDeviceClockRate(cudaIdx) / 1000 << " MHz");
    NR_DEBUG("The card has " << deviceProp.multiProcessorCount << " multiprocessors");

    isCardDoubleCapable = deviceProp.major > 1 || (deviceProp.major == 1 && deviceProp.minor > 2);

...
__kernel void blockMatchingKernel(
    ...
    if (currentBlockIndex > -1){
        ...
        const float targetMean = reduceCustom(sData, tid) / 64;
...
//		const float rTargetValue = targetInBounds ? targetImageArray[imgIdx] : NAN;
//
//		const float targetMean = reduceCustom(rTargetValue, tid) / 64;
//		const float targetTemp = rTargetValue - targetMean;
//		const float targetVar = reduceCustom(targetTemp*targetTemp, tid);
//
//		float bestDisplacement[3] = { NAN,0.0f,0.0f };
//		float bestCC = 0.0f;
//
...
//					const unsigned int bSize = (nBorder || mBorder || lBorder || border) ? (unsigned int)reduceCustom(overlap?1.0f:0.0f, tid) : 64;//out
//
//
//					if (bSize > 32 && bSize <= 64){
//
...
//
//							const float tChecked = overlap ? rTargetValue : 0.0f;
//							const float ttargetMean = reduceCustom(tChecked, tid) / bSize;
//							newTargetTemp = overlap ? tChecked - ttargetMean : 0.0f;
//							ttargetvar = reduceCustom(newTargetTemp*newTargetTemp, tid);
//						}
//
//						const float resultMean = reduceCustom(rChecked, tid) / bSize;
//						const float resultTemp = overlap ? rResultValue - resultMean : 0.0f;
//						const float resultVar = reduceCustom(resultTemp*resultTemp, tid);
//
//						const float sumTargetResult = reduceCustom((newTargetTemp)*(resultTemp), tid);
//						const float localCC = fabs((sumTargetResult) / sqrt(ttargetvar*resultVar));
//
//
//						if (tid == 0 && localCC > bestCC) {

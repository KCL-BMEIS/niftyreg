...
// Decompose the input tensor
	Eigen::JacobiSVD<Eigen::Matrix3d> svd(tensor,
...
template void svd<float>(float ** in, size_t m, size_t n, float * w,
		float ** v);
template void svd<double>(double ** in, size_t m, size_t n, double * w,
		double ** v);
/* *************************************************************** */
/* *************************************************************** */
template<class T>
void reg_LUdecomposition(T *mat, size_t dim, size_t *index) {
	...
	for (i = 0; i < dim; ++i) {
		...
		if (big == 0.f) {
			fprintf(stderr,
					"[NiftyReg] ERROR Singular matrix in the LU decomposition\n");
			reg_exit(1);
		}
		vv[i] = 1.0 / big;
	}
...
template void reg_matrixInvertMultiply<float>(float *, size_t, size_t *,
		float *);
template void reg_matrixInvertMultiply<double>(double *, size_t, size_t *,
		double *);
/* *************************************************************** */
/* *************************************************************** */
...
// First check that the dimension are appropriate
	if (dim1[1] != dim2[0]) {
		fprintf(stderr,
				"Matrices can not be multiplied due to their size: [%i %i] [%i %i]\n",
...
template void reg_matrixMultiply<float>(float *, float *, int *, int *,
		float * &);
template void reg_matrixMultiply<double>(double *, double *, int *, int *,
		double * &);
/* *************************************************************** */
/* *************************************************************** */
// Heap sort
...
float reg_mat44_det(mat44 const* A) {
	double D = (double) A->m[0][0] * A->m[1][1] * A->m[2][2] * A->m[3][3]
...
/* *************************************************************** */
template<class DTYPE>
void reg_mat44_mul(mat44 const* mat, DTYPE const* in, DTYPE *out) {
	out[0] = (DTYPE) mat->m[0][0] * in[0] + (DTYPE) mat->m[0][1] * in[1]
			+ (DTYPE) mat->m[0][2] * in[2] + (DTYPE) mat->m[0][3];
	out[1] = (DTYPE) mat->m[1][0] * in[0] + (DTYPE) mat->m[1][1] * in[1]
			+ (DTYPE) mat->m[1][2] * in[2] + (DTYPE) mat->m[1][3];
	out[2] = (DTYPE) mat->m[2][0] * in[0] + (DTYPE) mat->m[2][1] * in[1]
			+ (DTYPE) mat->m[2][2] * in[2] + (DTYPE) mat->m[2][3];
	return;
}
...
void reg_mat44_disp(mat44 *mat, char * title) {
	printf(
			"%s:\n%.4f\t%.4f\t%.4f\t%.4f\n%.4f\t%.4f\t%.4f\t%.4f\n%.4f\t%.4f\t%.4f\t%.4f\n%.4f\t%.4f\t%.4f\t%.4f\n",
			title, mat->m[0][0], mat->m[0][1], mat->m[0][2], mat->m[0][3],
...
void reg_mat33_disp(mat33 *mat, char * title) {
	printf("%s:\n%g\t%g\t%g\n%g\t%g\t%g\n%g\t%g\t%g\n", title, mat->m[0][0],
			mat->m[0][1], mat->m[0][2], mat->m[1][0], mat->m[1][1],
			mat->m[1][2], mat->m[2][0], mat->m[2][1], mat->m[2][2]);
}
/* *************************************************************** */
/* *************************************************************** */
